...
class ShipmentAdapter (var list: ArrayList<Shipment>, val callback: Callback) : RecyclerView.Adapter<ShipmentAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.textTitle.text = "Отгрузка № " + item.id
        //viewHolder.cardView.setOnClickListener { callback.onItemClicked(item) }

        viewHolder.textDescription.text = "Дата отгрузки: " +
                MyDateFormatter().formatDateToString(item.shipmentDate)
        viewHolder.imgDelete.setOnClickListener { callback.onItemDelete(item, position) }
    }

    interface Callback {
        //fun onItemClicked(product: Product)
        fun onItemDelete(item: Shipment, position: Int)
    }
}