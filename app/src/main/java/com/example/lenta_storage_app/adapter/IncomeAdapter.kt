...
class IncomeAdapter (var list: ArrayList<Income>, val callback: Callback) : RecyclerView.Adapter<IncomeAdapter.ViewHolder>() {
    ...
    override fun onBindViewHolder(viewHolder: ViewHolder, position: Int) {
        ...
        viewHolder.textTitle.text = "Поставка № " + item.id
        //viewHolder.cardView.setOnClickListener { callback.onItemClicked(item) }

        viewHolder.textDescription.text = "Дата поставки: " +
                MyDateFormatter().formatDateToString(item.incomeDate)
        viewHolder.imgDelete.setOnClickListener { callback.onItemDelete(item, position) }
    }

    interface Callback {
        //fun onItemClicked(product: Product)
        fun onItemDelete(item: Income, position: Int)
    }
}